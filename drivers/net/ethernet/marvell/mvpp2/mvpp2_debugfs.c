...
static int mvpp2_dbgfs_flow_type_show(struct seq_file *s, void *unused)
{
	...
	switch(f->flow_type) {
	case IPV4_FLOW: flow_name = "ipv4"; break;
	case IPV6_FLOW: flow_name = "ipv6"; break;
	case TCP_V4_FLOW: flow_name = "tcp4"; break;
	case TCP_V6_FLOW: flow_name = "tcp6"; break;
	case UDP_V4_FLOW: flow_name = "udp4"; break;
	case UDP_V6_FLOW: flow_name = "udp6"; break;
	default: flow_name = "unknown";
	}
					  ...
					  size_t len, loff_t *offp)
{
	...
	ai &= MVPP2_PRS_AI_MASK;
	ai_mask &= MVPP2_PRS_AI_MASK;

	mvpp2_prs_init_from_hw(entry->priv, &pe, entry->tid);

	pe.tcam[MVPP2_PRS_TCAM_AI_WORD] = ai | MVPP2_PRS_TCAM_AI_EN(ai_mask);
				 ...
				 struct mvpp2_port *port)
{
	...
	debugfs_create_file("rss_hits", 0444, port_dir, port,
