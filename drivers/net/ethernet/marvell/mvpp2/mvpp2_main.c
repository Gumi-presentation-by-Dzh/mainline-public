					    ...
					    struct mvpp2_tx_desc *tx_desc)
{
	if (port->priv->hw_version == MVPP21)
		...
		return le64_to_cpu(tx_desc->pp22.buf_dma_addr_ptp) & MVPP2_DESC_DMA_MASK;
}
					    ...
					    struct mvpp2_rx_desc *rx_desc)
{
	if (port->priv->hw_version == MVPP21)
		...
		return le64_to_cpu(rx_desc->pp22.buf_dma_addr_key_hash) & MVPP2_DESC_DMA_MASK;
}
					     ...
					     struct mvpp2_rx_desc *rx_desc)
{
	if (port->priv->hw_version == MVPP21)
		...
		return le64_to_cpu(rx_desc->pp22.buf_cookie_misc) & MVPP2_DESC_DMA_MASK;
}
